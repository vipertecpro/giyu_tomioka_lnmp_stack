@if(@$pageTitle || @$pageDescription)
    <div class="p-2 border border-gray-200 bg-gray-50 dark:bg-gray-800 dark:border-gray-700">
        @if(@$pageTitle)
        ...
        @endif
        <nav class="flex py-1 text-gray-700 flex-col md:flex-row justify-between" aria-label="Breadcrumb">
            ...
            @if(@$actions)
                <div class="space-x-2 flex justify-end pt-2 md:p-0">
                    @foreach(@$actions as $action)
                        <form action="{{ $action['route'] }}" method="POST">
                            @csrf
                            @method($action['method'])
                            <button type="submit" class="text-white bg-{{ $action['color'] }}-700 hover:bg-{{ $action['color'] }}-800 focus:ring-2 focus:outline-none focus:ring-{{ $action['color'] }}-300 font-medium rounded-md text-sm p-1 md:p-2 text-center inline-flex items-center dark:bg-{{ $action['color'] }}-600 dark:hover:bg-{{ $action['color'] }}-700 dark:focus:ring-{{ $action['color'] }}-800">
